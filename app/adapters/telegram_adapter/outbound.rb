    ...
    class << self
      def send!(message)
        ...
        if files.present?
          ...
          TelegramAdapter::Outbound::Photo.perform_later(organization_id: message.request.organization.id,
                                                         contributor_id: recipient.id,
                                                         media: media,
                                                         message: message)
        else
          TelegramAdapter::Outbound::Text.perform_later(organization_id: message.request.organization.id,
                                                        contributor_id: recipient.id,
                                                        text: message.text,
                                                        message: message)
        end
