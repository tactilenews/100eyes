  ...
  class ChatMessage < ApplicationComponent
    ...
    def sent_by_reference
      ...
      if message.sent_from_contributor?
        link_to_unless(current_page?(conversations_organization_contributor_path(organization_id: message.organization_id, id: message.contributor.id), check_parameters: false), sent_by_x_at, conversations_organization_contributor_path(message.organization, id: message.contributor.id, anchor: id), data: { turbo: false })
      else
