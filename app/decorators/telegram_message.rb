...
class TelegramMessage
  ...
  def initialize_photos(telegram_message)
    ...
    photo.attachment.attach(io: URI.parse(remote_file_location.to_s).open, filename: File.basename(remote_file_location.path))
    [photo]
  end
  ...
  def initialize_voice(telegram_message)
    ...
    voice.attachment.attach(io: URI.parse(remote_file_location.to_s).open, filename: File.basename(remote_file_location.path))
    voice
  end
  ...
  def retrieve_message_type_and_attach(telegram_file)
    ...
    response = JSON.parse(URI.parse(uri.to_s).open.read)
