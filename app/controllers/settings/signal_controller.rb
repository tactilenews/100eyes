...
module Settings
  class SignalController < ApplicationController
    ...
    def register
      uri = URI.parse("#{Setting.signal_cli_rest_api_endpoint}/v1/register/#{signal_server_phone_number}")
    ...
    def verify
      ...
      uri = URI.parse("#{Setting.signal_cli_rest_api_endpoint}/v1/register/#{signal_server_phone_number}/verify/#{token}")
      request = Net::HTTP::Post.new(uri, {
                                      Accept: 'application/json',
                                      'Content-Type': 'application/json'
                                    })
