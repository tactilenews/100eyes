...
RSpec.describe 'Conversation interactions', js: false do
  ...
  before do
    ...
    visit conversations_organization_contributor_path(contributor.organization_id, contributor, as: user)
  end
  ...
  it 'can navigate to the requests view and back' do
    ...
    link_to_conversations = conversations_organization_contributor_path(contributor.organization_id, id: contributor.id, anchor: "message-#{first_received_message.id}")
    find("a[href='#{link_to_conversations}']").trigger('click')

    expect(page).to have_current_path(conversations_organization_contributor_path(contributor.organization_id, contributor))
  end
  ...
  it 'can answer to a previous message and assigns the message to its request', flaky: true do
    ...
    reply_path = conversations_organization_contributor_path(contributor.organization_id, contributor, reply_to: sent_message.id, anchor: 'chat-form')
