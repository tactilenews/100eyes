...
RSpec.describe 'Conversation interactions', js: false do
  ...
  it 'can answer to the latest message and assigns the message to its request' do
    last_message_request_path = request_path(last_received_message.request, anchor: "contributor-#{contributor.id}")
    ...
    expect(page).to have_link(nil, href: last_message_request_path, count: 2)
  end
  ...
  it 'can answer to a previous message and assigns the message to its request', flaky: true do
    ...
    sent_message_request_path = request_path(sent_message.request, anchor: "contributor-#{contributor.id}")
    ...
    expect(page).to have_text(reply_text)
    find('p', text: reply_text)
    expect(page).to have_link(nil, href: sent_message_request_path, count: 2)
  end
end
