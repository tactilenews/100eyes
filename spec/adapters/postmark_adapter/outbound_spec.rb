    ...
    describe '#welcome_email' do
      ...
      before do
        ...
        allow(onboarding_success_heading_record).to receive(:send)
          .with("value_#{I18n.locale}".to_sym).and_return('Welcome new contributor!')
        allow(Setting).to receive(:find_by)
          .with(var: :onboarding_success_text)
          .and_return(onboarding_success_text_record)
        allow(onboarding_success_text_record).to receive(:send)
          .with("value_#{I18n.locale}".to_sym).and_return('You onboarded successfully.')
      end
